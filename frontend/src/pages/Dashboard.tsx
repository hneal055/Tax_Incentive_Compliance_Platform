...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-[28px] font-bold text-slate-900 tracking-tight">Executive Dashboard</h1>
        <p className="text-slate-500 mt-1.5 text-[15px]">Overview of active productions and tax incentive performance.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        {metrics.map((metric) => {
          ...
          return (
            <div key={metric.title} className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 hover:shadow-md transition-shadow">
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-slate-500 text-sm font-medium">{metric.title}</p>
                  <p className="text-[32px] font-bold text-slate-900 mt-2 leading-none tracking-tight">{metric.value}</p>
                  <p className="text-[13px] text-slate-400 mt-2 font-medium">{metric.subtitle}</p>
                </div>
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-white shrink-0 ${metric.iconClass}`}>
