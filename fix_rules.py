...
try:
    ...
    if program:
        program.rules = json.dumps(corrected_rules)
        db.commit()
        print("✅ Georgia rules updated successfully!")
        print(f"   Program ID: {program.id}")
        print(f"   Base Credit Rate: {corrected_rules['base_credit_rate'] * 100}%")
        print(f"   Qualified Categories: {len(corrected_rules['qualified_expenditure_categories'])}")
        print(f"   Checklist Items: {len(corrected_rules['checklist'])}")
    else:
